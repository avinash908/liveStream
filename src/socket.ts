...
export const socketServer = (io: Server) => {
    ...
    io.on("connection", (socket: Socket) => {
        ...
        socket.on("produce", async ({ kind, rtpParameters, producerTransportId, id }, cb) => {
            if (!rooms!.has(id)) {
                return cb({
            ...
            let producer_id = await rooms!.get(id)?.produce(socket.id, producerTransportId, rtpParameters, kind, id)
            console.log('Produce', {
                type: `${kind}`,
                name: `${rooms!.get(id)?.getPeers()!.get(socket.id)?.name}`,
                id: `${producer_id}`
        ...
        socket.on('consume', async ({ rtpCapabilities, consumerTransportId, producerId, roomId }, callback) => {
            if (!rooms!.has(roomId)) {
